...
static void setup() {
  ...
  fail_unless((process = process_init()) != NULL);
}
...
START_TEST(prepare_null_command) {
  fail_unless(process_prepare(process, NULL) == NULL);
}
END_TEST
...
START_TEST(prepare_no_args) {
  ...
  fail_unless((process_info = process_prepare(process, "foobar")) != NULL);
...
START_TEST(prepare_one_arg) {
  ...
  fail_unless((process_info = process_prepare(process, "foobar foo")) != NULL);
  fail_unless(strcmp(process_info_get_command(process_info), "foobar") == 0);
...
START_TEST(prepare_two_args) {
  ...
  fail_unless((process_info = process_prepare(process, "foobar foo bar")) != NULL);
  fail_unless(strcmp(process_info_get_command(process_info), "foobar") == 0);
  fail_unless(strcmp(process_info_get_args(process_info)[0], "foo") == 0);
...
START_TEST(exec_null_process_info) {
  fail_unless(process_exec(process, NULL, session) == -1);
}
END_TEST
...
START_TEST(exec_null_session) {
  ...
  fail_unless((process_info = process_prepare(process, "foobar")) != NULL);
  fail_unless(process_exec(process, process_info, NULL) == -1);
  fail_unless(process_info_destroy(process_info) == 0);
}
END_TEST
...
START_TEST(exec_absolute_path) {
  ...
  fail_unless((process_info = process_prepare(process, "/bin/ls -1")) != NULL);
  fail_unless(process_exec(process, process_info, session) == 0);
...
START_TEST(exec_relative_path) {
  ...
  libssh_proxy_set_option_int("ssh_select", "readfds", 1);
  fail_unless((process_info = process_prepare(process, "ls -1")) != NULL);
  fail_unless(process_exec(process, process_info, session) == 0);
  fail_unless(libssh_proxy_channel_get_size(session->channel) > 0);
  fail_unless(process_info_destroy(process_info) == 0);
}
...
START_TEST(exec_no_such_file) {
  ...
  libssh_proxy_set_option_int("ssh_select", "readfds", 1);
  fail_unless((process_info = process_prepare(process, "foobar")) != NULL);
  fail_unless(process_exec(process,process_info, session) == 127);
  fail_unless(libssh_proxy_channel_get_size(session->channel) == 0);
  fail_unless(process_info_destroy(process_info) == 0);
}
END_TEST
